public class Predictor {
    ...
    public static double[][] createPredictTable(int numberOfGoalsPrediction,
        ...
        for (int i = 0; i < numberOfGoalsPrediction; i++)
            System.out.printf("| %-8s", Formatting.form2(predictionScoresHomeArray[i] * 100) + "%");
        System.out.println();
        ...
        for (double[] i : results) {
            ...
            System.out.printf("| %-8s", Formatting.form2(predictionScoresAwayArray[n++] * 100) + "%");
            for (double j : i) {

                System.out.printf("| %-8s", Formatting.form(j * 100) + "%");
            }
            System.out.println();
        }
        return results;
    ...
    public static double[] getPredictionsArray(int numberOfGoalsPrediction, double TeamPredictedNumberOfGoals) {
        ...
        for (int i = 0; i < numberOfGoalsPrediction; i++) {
            ...
            System.out.println("For " + i + " goals prediction is " + Formatting.form2(predictionScoresAway[i] * 100) + "%");
        }
        return predictionScoresAway;
    }


    public static void printFinalPredictedScore(int numberOfGoalsPrediction, double[][] results, TeamData data) {
        double max = 0;
        int x = 0;
        int y = 0;
        for (int i = 0; i < numberOfGoalsPrediction; i++) {
